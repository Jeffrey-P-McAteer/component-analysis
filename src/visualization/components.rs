#[cfg(feature = "gui")]
use crate::types::{Component, Relationship, AnalysisResult, AnalysisType, ComponentType};
#[cfg(feature = "gui")]
use crate::database::{RelationshipQueries, AnalysisQueries};
#[cfg(feature = "gui")]
use crate::visualization::documentation::DocumentationRenderer;
#[cfg(feature = "gui")]
...
impl ComponentDetailView {
    pub fn new() -> Self {
        Self {
            ...
            documentation_renderer: DocumentationRenderer::new(),
        }
    }
    ...
    pub fn set_component(
        ...
        // Clear documentation when component changes
        self.documentation_renderer.clear_state();
        
        // If this is a function component, try to load cached documentation
        if component.component_type == ComponentType::Function {
            if let Err(e) = self.documentation_renderer.load_cached_documentation(&component.name, conn) {
                log::error!("Failed to load cached documentation for {}: {}", component.name, e);
            }
        }
        ...
        if let Some(component) = &self.component {
            ...
            if component.component_type == ComponentType::Function {
                CollapsingHeader::new("Function Documentation").show(ui, |ui| {
                    self.documentation_renderer.render_section(ui, Some(&component.name), Some(&component.id), Some(component.component_type.clone()), db_conn);
                });
            }
        } else {
...
impl FilterPanel {
    ...
    pub fn render(&mut self, ui: &mut Ui) -> bool {
            ...
            ComboBox::from_label("")
                ...
                .show_ui(ui, |ui| {
                    ...
                    for component_type in &[
                        ...
                        if ui.selectable_value(&mut self.component_type_filter, Some(component_type.clone()), component_type.to_string()).changed() {
                            changed = true;
                        }
                    }
                });
        ...
        ui.horizontal(|ui| {
            ui.label("Name:");
            if ui.add(TextEdit::singleline(&mut self.name_filter).hint_text("Filter by name")).changed() {
                changed = true;
            }
        });
            ...
            ComboBox::from_label("")
                ...
                .show_ui(ui, |ui| {
                    ...
                    for analysis_type in &[
                        ...
                        if ui.selectable_value(&mut self.analysis_type_filter, Some(analysis_type.clone()), analysis_type.to_string()).changed() {
                            changed = true;
                        }
                    }
                });
        ...
        ui.horizontal(|ui| {
            ...
            ComboBox::from_label("")
                ...
                .show_ui(ui, |ui| {
                    if ui.selectable_value(&mut self.has_relationships_filter, None, "All").changed() {
                        changed = true;
                    }
